class Solution {
    // refer codestorywithMIK + ChatGPT o3 
    // BRUTE/BETTER
    // can be done with Greedy as well.
    // T: O(m); O(m) for `m` tasks + 26log26 for heap
    // S: O(26); 
    /*
     ...
     * Add all non-zero frequencies to the heap initially.
     * 6. Loop over the heap and process the non-zero freq. elements.
     * 7. In this, process the elements in a cycle. The cycle length = n+1. So process until the current cycle is complete.
     * Poll element, reduce its frequency and add the non-zero frequency elements to a list.
     * 8. After the cycle is complete or heap is empty, add the elements from the list to the heap for processing in next cycle.
     * 9. If there are no tasks left, then heap is empty, so break.
     * 10. Everytime, add the remaining cycle to the total time because it indicates the idle time.
     */
